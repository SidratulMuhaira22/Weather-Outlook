...
import com.hera.weatheroutlook.databinding.ActivityMainBinding
...
class MainActivity : AppCompatActivity() {
    ...
    private val listAdapter = WeatherItemListAdapter()
    ...
    private fun weatherResultListener() {
        lifecycleScope.launchWhenStarted {
            mainActivityViewModel.weatherNetworkResult.collectLatest {
                binding.loading.isVisible = it is NetworkResult.Loading
                if (it is NetworkResult.Error) showError(it.localizedMessage)
    ...
    private fun currentLocationListener() {
        lifecycleScope.launchWhenStarted {
            mainActivityViewModel.currentLocation.collectLatest {
                binding.loading.isVisible = it is NetworkResult.Loading
                if (it is NetworkResult.Error) showError(it.localizedMessage)
                else if (it is NetworkResult.Success) mainActivityViewModel.fetchWeatherFromDatabase()
            }
        }
    }
